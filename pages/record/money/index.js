...
Page({
  ...
  onLoad() {
      const soupList = soupListStr.filter((item) => item.length >=12);
      console.log('==soupList==',soupList)
  ...
  exec() {
    const index = Math.round(Math.random() * (this.data.soupList.length));
    this.setData({
        dishName: this.data.soupList[index]
    })
    return
